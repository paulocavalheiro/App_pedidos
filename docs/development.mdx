...
Navigate to your **docs directory** (the folder that contains `docs.json`). For this project, use the `docs` folder:
...
<Warning>
  Run `mint dev` from inside the **docs** folder only. Running it from the
  project root can cause "module is not defined" errors because Mintlify would
  try to bundle backend and frontend code.
</Warning>
...
<Accordion title="Issue: Encountering an unknown error">
  ...
  <Accordion title='Error: "module is not defined" or "exports is not defined" in browser'>
    Run `mint dev` from inside the **docs** folder (`cd docs` then `mint dev`), not from the project root. The docs folder contains only documentation files; the project root has backend and frontend code that must not be bundled by Mintlify.
  </Accordion>
</AccordionGroup>
